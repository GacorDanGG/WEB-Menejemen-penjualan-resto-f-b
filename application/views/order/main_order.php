...
<nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-dark sidebar collapse" style="min-height: 100vh;">
    ...
    <div class="flex-grow-1 p-4" style="background-color: #f8f9fa;">
        <h2>Orders List</h2>
        <a href="<?= site_url('order_input/add_order'); ?>" class="btn btn-primary mb-3">Tambah Order</a>
                ...
                <?php if (!empty($orders)) : ?>
                    <?php foreach ($orders as $order) : ?>
                        <tr>
                            ...
                            <td><?= $order['user_id']; ?></td>
                            <td>
                                <a href="<?= site_url('order_input/delete/'.$order['order_id']); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this order?')">Hapus</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else : ?>
                    <tr><td colspan="6">Tidak ada data order</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
